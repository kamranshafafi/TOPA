...
main(int argc, char* argv[])
{
    ...
    for (size_t i = 0; i < LoSList.size(); ++i)
    {
...

        {
            ...
            std::cout << "StaNodeId: " << wifiStaNodes.Get(i)->GetId() << std::endl;
            SetTxPower(wifiStaNodes.Get(i)->GetId(), (16.0206 - NLoSLoss[i] + LoSLoss[i]));
            std::cout << "NLOS with ue # :" << wifiStaNodes.Get(i)->GetId() << std::endl;
        }
    }
    for (uint32_t nodeId = 1; nodeId < wifiStaNodes.GetN(); nodeId++)
    {
        std::cout << "the Nodes is " << wifiStaNodes.GetN() << std::endl;
