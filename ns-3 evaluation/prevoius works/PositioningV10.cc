...
main(int argc, char* argv[])
{
    ...
    else
    {
        ...
        wifiChannel->SetPropagationLossModel(lossModel1);
        std::clog << "Propagation Loss Model is: ItuR1411LosPropagationLossModel BUT....."
                  << std::endl;
    }
    ...
    for (int i = 0; i < LoSList.size(); ++i)
    {
        ...
        {
            SetTxPower(wifiStaNodes.Get(i)->GetId(), 0.0);
            std::cout << "NLOS with ue # :" << wifiStaNodes.Get(i)->GetId() << std::endl;
        }
    }

    // Config::Set("/NodeList/*/DeviceList/*/$ns3::WifiNetDevice/Phy/TxPowerStart",
    // DoubleValue(0.0)); Config::Set("/NodeList/*/DeviceList/*/$ns3::WifiNetDevice/Phy/TxPowerEnd",
    // DoubleValue(0.0));

    //  if (!Config::SetFailSafe(
    //                     "/NodeList/*/DeviceList/*/$ns3::WifiNetDevice/Phy/TxPowerStart",
    //                     DoubleValue(0.0)))
    //             {
    ...
    //             }
    //             if (!Config::SetFailSafe(
    //                     "/NodeList/*/DeviceList/*/$ns3::WifiNetDevice/Phy/TxPowerEnd",
    //                     DoubleValue(0.0)))
    //             {
    //                 std::cerr << "Failed to set : "
    //                              "/NodeList/*/DeviceList/*/$ns3::WifiNetDevice/Phy/TxPowerEnd"
