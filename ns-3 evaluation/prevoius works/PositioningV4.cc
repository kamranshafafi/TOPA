...
main (int argc, char *argv[])
{
  ...
  ue1->SetPosition (Vector (0.0, 20.0, 0.1));
  ue2->SetPosition (Vector (0.0, -25.0, 0.1));
  ...
  if (cond1->GetLosCondition () == ChannelCondition::LosConditionValue::LOS &&
      cond2->GetLosCondition () == ChannelCondition::LosConditionValue::LOS)
    {
      ...
      // LossModel->SetAttribute ("MinLoss", DoubleValue (0));
  ...
  Simulator::Run ();
  monitor->SerializeToXmlFile ("positioningflow4.xml", true, true);
