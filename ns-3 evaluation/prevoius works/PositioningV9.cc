...
main(int argc, char* argv[])
{
    ...
    Ptr<PropagationLossModel> lossModel{};
        ...
        if (nLos == nSta) // All have LOS
    {
        Ptr<ItuR1411LosPropagationLossModel> lModel =
        ...
        for (uint32_t i{}; i < nSta; ++i)
        {
            ue = wifiStaNodes.Get(i)->GetObject<ConstantPositionMobilityModel>(
                ConstantPositionMobilityModel::GetTypeId());
            double loss = lModel->GetLoss(ap, ue);
            std::cout << "Loss(AP, UE#" << i << "): " << loss << std::endl;
        }

        std::clog << "ItuR1411LosPropagationLossModel" << std::endl;
    }
    else if (nLos)
    {
        Ptr<ItuR1411NlosOverRooftopPropagationLossModel> lModel =
        ...
        lossModel->SetAttribute("StreetsOrientation", DoubleValue(45));
        lossModel->SetAttribute("CitySize", StringValue("Small"));

        lossModel->SetAttribute("BuildingSeparation", DoubleValue(100));
        lossModel->SetAttribute("BuildingsExtend", DoubleValue(10));
        lossModel->SetAttribute("StreetsWidth", DoubleValue(200));
    }
    else
    {
        Ptr<ItuR1411LosPropagationLossModel> lModelA =
            CreateObject<ItuR1411LosPropagationLossModel>();
        Ptr<ItuR1411NlosOverRooftopPropagationLossModel> lModelB =
            CreateObject<ItuR1411NlosOverRooftopPropagationLossModel>();
        ...
        if (nLos) // Some have LOS
        {
            std::clog << "Some have LOS" << std::endl;
        }
        else
        {
            std::clog << "ItuR1411NlosOverRooftopPropagationLossModel" << std::endl;
        }
    }
