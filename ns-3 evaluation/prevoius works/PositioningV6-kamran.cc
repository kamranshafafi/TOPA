...
main(int argc, char* argv[])
{
    ...
    ap->SetPosition(Vector(0.0, 10.48, 11.0));
    ue1->SetPosition(Vector(0.0, 20.0, 1));
    ue2->SetPosition(Vector(0.0, -15.0, 1));
    ...
    double averageThroughput =
        ((sink->GetTotalRx() * 8) / (1e6 * (Simulation_Stop_Time - Simulation_Start_Time)));
    ...
    std::cout << "Tx Bytes = " << txPacketCounter << std::endl;
